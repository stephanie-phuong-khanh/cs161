...
; Helper to at-most-one-color function written below
(defun at-most-one-color-helper (n c1 c2 k)
    (if (> c2 k)
        ()
        (cons (list (- (node2var n c1 k)) (- (node2var n c2 k))) (at-most-one-color-helper n c1 (+ c2 1) k))
    )
)
; EXERCISE: Fill this function
...
; Helper to enerate-edge-clauses function written below
(defun generate-edge-clauses-helper (l1 l2)
    (if (or (null l1) (null l2))
        ()
        (cons (list (- (car l1)) (- (car l2))) (generate-edge-clauses-helper (cdr l1) (cdr l2)))
    )
)
; EXERCISE: Fill this function
; returns *a list of clauses* to ensure that
...
(defun generate-edge-clauses (e k)
    (generate-edge-clauses-helper (at-least-one-color (first e) 1 k) (at-least-one-color (second e) 1 k))
)
...
;; (graph-coloring-to-sat "graph1.txt" "sat1.txt" 3)
;; (graph-coloring-to-sat "graph1.txt" "sat2.txt" 4)
;; (graph-coloring-to-sat "graph2.txt" "sat3.txt" 8)